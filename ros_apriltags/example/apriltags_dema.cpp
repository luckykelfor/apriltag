...
//rest is for cv_bridge and ros and geometry_msgs
#include <ros/ros.h>
...
const char* windowName = "apriltags_detection";
...
public:
  ...
  void publish(const geometry_msgs::PoseWithCovarianceStamped& msg) const{
    ...
    : it_(nh_)
  {
    ...
    image_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("apriltag_pose/pose", 1000);
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    //if(!process_flag){
      try
      {
        ...
        cv_ptr->image.copyTo(rst_image);
        //cout<<rst_image.cols<<"    "<<rst_image.rows<<endl;
      }
    ...
    /*if (cv_ptr->image.rows > 60 && cv_ptr->image.cols > 60)
      cv::circle(cv_ptr->image, cv::Point(cv_ptr->image.cols/2,cv_ptr->image.rows/2), min(cv_ptr->image.rows, cv_ptr->image.cols)/4, CV_RGB(255,0,0));
    // Update GUI Window
...
public:
  ...
  void loop() {
    ...
    double begin = tic();
    ros::spinOnce();
    //cv::pyrDown(ic.rst_image, dst, cv::Size( ic.rst_image.cols/2, ic.rst_image.rows/2 ) );
    while (true) {
      ...
      ic.rst_image.copyTo(dst);
      //cv::pyrDown(dst, dst2, cv::Size( dst.cols/2, dst.rows/2 ) );
      if(!dst.empty()){
        //cv::pyrDown(dst, dst2, cv::Size( dst.cols/2, dst.rows/2 ) );
        cv::resize(dst,dst2, cv::Size(dst.cols/2, dst.rows/2) );
        cout<<dst.cols<<"    "<<dst.rows<<endl;
        cout<<dst2.cols<<"    "<<dst2.rows<<endl;
        processImage(image,dst2);
      }
      //processImage(image, image_gray);
      ...
      if (frame % 10 == 0) {
        double t = tic();
        cout<<endl<<endl;
        cout << "  " << 10./(t-last_t) << " fps " <<"hahahaha"<< endl;
        cout<<endl<<endl;
        last_t = t;
      }
      ...
      if (cv::waitKey(1) >= 0) {
      	double final_time = tic();	
      	cout<<"average rate:"<<frame/(final_time - begin)<<" fps"<<endl;
      	break;
      }
    }
  }
...
/* here starts the inageConverter class */
